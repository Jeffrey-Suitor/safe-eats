...
static esp_err_t FlashGetConsole(const char *key, const char *str_type) {
  nvs_handle_t nvs;
  esp_err_t err;
...
static int ListItems(const char *part, const char *name, const char *str_type) {
  nvs_type_t type = StrToType(str_type);
...
static int SetValue(int argc, char **argv) {
  ...
  esp_err_t err = FlashSetConsole(key, type, values);
...
static int GetValue(int argc, char **argv) {
  ...
  esp_err_t err = FlashGetConsole(key, type);
...
static int SetNamespace(int argc, char **argv) {
  ...
  strlcpy(NAMESPACE, namespace, sizeof(NAMESPACE));
  ESP_LOGI(TAG, "Namespace set to '%s'", NAMESPACE);
  return 0;
}
...
static int ListEntries(int argc, char **argv) {
  ...
  return ListItems(part, name, type);
}

void RegisterFlash(void) {
  ...
  ESP_ERROR_CHECK(esp_console_cmd_register(&ListEntries_cmd));
  ESP_ERROR_CHECK(esp_console_cmd_register(&EraseNamespace_cmd));
}
...
esp_err_t request(char *url, char *method_string,
                  esp_http_client_method_t method, char *post_data, char *buf) {
  ...
  if (err == ESP_OK) {
    ESP_LOGD(TAG, "HTTPS %s status = %d", method_string,
             esp_http_client_get_status_code(client));
  } else {
    ESP_LOGE(TAG, "HTTPS %s request failed: %s", method_string,
             esp_err_to_name(err));
  }
...
void SetupDBManager(void) {
  ...
  xTaskCreate(PostTemperatureTask, "PostTemperatureTask", 4096 * 2, NULL, 3,
              NULL);
  xTaskCreate(DefineInDatabaseTask, "DefineInDatabaseTask", 4096 * 2, NULL, 2,
              NULL);

...
void BuzzerTask(void *pvParameters) {
  BuzzerNote note;
  while (true) {
    ESP_LOGI(TAG, "Received note with duration %d and freq %d", note.duration,
             note.freq);
...
static int BuzzerConsoleCmd(int argc, char **argv) {
  ...
  sound(console_note_args.freq->ival[0], console_note_args.duration->ival[0],
        console_note_args.repeats->ival[0]);
  return 0;
}

void RegisterBuzzer(void) {
  console_note_args.freq =
      arg_int1(NULL, NULL, "<freq>", "Frequency of the sounds");
  console_note_args.duration =
      arg_int1(NULL, NULL, "<duration>", "Duration of sound in ms");
  console_note_args.repeats =
      arg_int1(NULL, NULL, "<repeats>", "Number of times the sound repeats");
